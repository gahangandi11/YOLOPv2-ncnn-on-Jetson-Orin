...
static int detect_yolopv2( cv::Mat& bgr, std::vector<Object>& objects, ncnn::Mat& da_seg_mask_, ncnn::Mat&  ll_seg_mask_,ncnn::Net& yolopv2)
{
    // ncnn::Net yolopv2;
...
int main(int argc, char** argv)
{
    ...
    detect_yolopv2(frame, objects, da_seg_mask, ll_seg_mask,yolopv2);
    auto stop = std::chrono::high_resolution_clock::now();
    draw_objects(frame, objects,da_seg_mask, ll_seg_mask);
    

    // Calculate the duration in seconds
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);

    std::cout << "Time taken by code: " << duration.count() << " milli seconds" << std::endl;
    }
